...
export function InitialSplash() {
  ...
  useEffect(() => {
    ...
    const seen = sessionStorage.getItem('figo-splash-seen');
    if (seen) {
      setPhase('hidden');
      return;
    }
    ...
    const hide = window.setTimeout(() => {
      setPhase('hidden');
      try {
        sessionStorage.setItem('figo-splash-seen', '1');
      } catch {
  ...
  return (
    <div
      ...
      className={
        'fixed inset-0 z-[80] flex flex-col items-center justify-center bg-ink transition-opacity duration-700 ease-out ' +
    ...
    >
      ...
      <div className="flex flex-col items-center gap-10">
        <Image
          ...
          height={52}
          priority
          className="h-12 w-auto brightness-0 invert animate-pulse-soft"
        />
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="relative h-px w-44 overflow-hidden rounded-full bg-white/10">
            <div
              ...
              style={{
                animation:
                  'splashBar 1.4s cubic-bezier(0.45,0.05,0.55,0.95) infinite',
              }}
            />
          </div>
